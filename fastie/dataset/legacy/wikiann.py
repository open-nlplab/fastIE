...
class WikiannLoaderConfig(BaseDatasetConfig):
    language: str = field(default="zh",
                          metadata=dict(
                              help="Select which language subset in wikiann. Refer to https://huggingface.co/datasets/wikiann .",
                              existence=True
                          ))
...
class WikiannLoader(BaseDataset):
    ...
    def run(self):
        ...
        if get_flag() == "train" or get_flag() == "evaluation":
            datasets["train"] = DataSet([Instance(tokens=sample["tokens"], ner_tags=sample["ner_tags"])
                                         for sample in raw_dataset["train"]])
            datasets["evaluate"] = DataSet([Instance(tokens=sample["tokens"], ner_tags=sample["ner_tags"])
