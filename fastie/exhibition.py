...
class Exhibition:
    """
    FastIE 展示模块,
    主要用来在命令行模式中展示 FastIE 现有的 ``dataset``, ``task`` 和 ``config``
    """
    ...
    def RE(self):
        ...
        for key, value in RE.module_dict.items():
            table.append(
                dict(task='RE', solution=key, description=value().description))
        return table
    ...
    def EE(self):
        ...
        for key, value in EE.module_dict.items():
            table.append(
                dict(task='EE', solution=key, description=value().description))
        return table
    ...
    def DATASET(self):
        ...
        for key, value in DATASET.module_dict.items():
            table.append(dict(dataset=key, description=value().description))
        return table
    ...
    def intercept(cls):
        if '-l' in sys.argv or '--list' in sys.argv:
            ...
            for i in range(len(sys.argv)):
                if sys.argv[i].startswith('--task') or sys.argv[i].startswith(
                        '-t'):
                    if i < len(sys.argv) - 1 and sys.argv[i + 1].upper(
                    ).replace('/', '').replace('\\', '') == 'NER':
                        table.add_rows([[
                    ...
                    elif i < len(sys.argv) - 1 and sys.argv[i + 1].upper(
                    ).replace('/', '').replace('\\', '') == 'EE':
                        table.add_rows([[
                            'Task', 'Solution', 'Description'
                        ], *[[
                    ...
                    exit(0)
                        ...
                        '--dataset') or sys.argv[i].startswith('-d'):
                    ...
                    exit(0)
                        ...
                        '--config') or sys.argv[i].startswith('-c'):
