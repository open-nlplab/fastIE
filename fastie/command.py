    ...
    for key in keys:
        if key == "task":
            ...
            if obj_cls is not None:
                ...
                print(f"The solution `{solution}` you selected does not exist. ")
            ...
            if obj_cls is None:
                print(f"The dataset `{global_config[key]}` you selected does not exist. ")
                print("Here are the optional options: \n")
                sys.argv.append("-d")
                sys.argv.append("-l")
                Exhibition.intercept()
...
class CommandNodeConfig(BaseNodeConfig):
    ...
    task: str = field(default="",
                      metadata=dict(
                          help="The task you want to use. Please use / to split the task and the specific solution.",
                          existence=True,
                          alias="-t"
                      ))
            ...
            def __call__(self, parser: ArgumentParser,
                ...
                if variable_name == "task":
                    if '/' in values:
                        ...
                        print(f"You must specify both the task category and the specific solution, such as `ner/bert` instead of `{values}`. ")
                        print("Here are the optional options: \n")
                        sys.argv.append("-t")
                        sys.argv.append("-l")
                        Exhibition.intercept()
                    ...
                    if obj_cls is None:
                        print(f"The dataset `{values}` you selected does not exist. ")
                        print("Here are the optional options: \n")
                        sys.argv.append("-d")
                        sys.argv.append("-l")
                        Exhibition.intercept()
                    ...
                    if os.path.exists(values):
                        ...
                        if config_flag == "class":
                            config = Config.fromfile(find_config(values))["Config"]()
                        elif config_flag == "dict":
                            config = Config.fromfile(find_config(values))["config"]
                        set_config(config)
                        parse_config()
                        ...
                        print(f"The config file `{values}` you selected does not exist. ")
                        print("Here are the optional options: \n")
                        sys.argv.append("-c")
                        sys.argv.append("-l")
                        Exhibition.intercept()
        ...
        if sys.argv[i] == "-c" or sys.argv[i] == "--config":
            if i < len(sys.argv) - 1:
                if os.path.exists(sys.argv[i + 1]):
                    ...
                    set_config(config)
                elif os.path.exists(os.path.join(FASTIE_HOME, sys.argv[i + 1])):
                    config = Config.fromfile(os.path.join(FASTIE_HOME, sys.argv[i]))["Config"]()
                    set_config(config)
                break
